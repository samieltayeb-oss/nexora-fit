...
export default function AdminExerciseReviewPage() {
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 font-sans p-4 md:p-8 space-y-6">
      ...
      {/* Header */}
      <div className="flex items-center justify-between border-b border-slate-800 pb-5 max-w-6xl mx-auto">
        <div>
          ...
          <h1 className="text-3xl font-bold text-white tracking-tight mt-1">Exercise Content & Visual Audit</h1>
        </div>
        <Link href="/dashboard" className="px-4 py-2 bg-slate-900 border border-slate-800 rounded-xl text-xs font-semibold hover:text-white text-slate-300">
          Back to App
        </Link>
      </div>
      ...
      <div className="max-w-6xl mx-auto space-y-6">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 space-y-1">
            ...
            <div className="text-2xl font-bold text-white">{totalCount} <span className="text-xs text-slate-400 font-normal">Exercises</span></div>
          </div>
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 space-y-1">
            <span className="text-slate-500 text-xs uppercase font-semibold">Documented Benchmark</span>
            <div className="text-2xl font-bold text-teal-400">{benchmarkCount} <span className="text-xs text-slate-400 font-normal">Active</span></div>
          </div>
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 space-y-1">
            <span className="text-slate-500 text-xs uppercase font-semibold">Media Approved</span>
            <div className="text-2xl font-bold text-emerald-400">{approvedCount} <span className="text-xs text-slate-400 font-normal">Ready</span></div>
          </div>
          <div className="bg-slate-900 border border-slate-800 rounded-2xl p-4 space-y-1">
            <span className="text-slate-500 text-xs uppercase font-semibold">Pending Review</span>
            <div className="text-2xl font-bold text-amber-400">{needsReviewCount} <span className="text-xs text-slate-400 font-normal">Review</span></div>
          </div>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-6">
          ...
          <div className="bg-slate-900 border border-slate-800 rounded-3xl p-4 space-y-3">
            <div className="flex items-center justify-between border-b border-slate-800 pb-3">
              <h3 className="font-bold text-white text-sm">Exercises ({exercises.length})</h3>
              
              <div className="flex gap-1 bg-slate-950 p-1 rounded-xl border border-slate-800 text-[10px]">
                <button 
                  onClick={() => setFilterStatus('all')}
                  className={`px-2 py-1 rounded-lg font-semibold ${filterStatus === 'all' ? 'bg-slate-800 text-teal-400' : 'text-slate-500'}`}
                >
                  All
                </button>
                <button 
                  onClick={() => setFilterStatus('approved')}
                  className={`px-2 py-1 rounded-lg font-semibold ${filterStatus === 'approved' ? 'bg-slate-800 text-teal-400' : 'text-slate-500'}`}
                >
                  Approved
                </button>
              </div>
            ...
            <div className="space-y-2">
              {exercises.map((ex) => (
                ...
                >
                  ...
                  <span className="text-[11px] text-slate-400 block mt-1">{ex.equipment}</span>
                </button>
              ))}
            </div>
          </div>
          ...
          <div className="md:col-span-2 bg-slate-900 border border-slate-800 rounded-3xl p-6 space-y-6">
            ...
            <div className="flex justify-between items-start border-b border-slate-800 pb-4">
              <div>
                <h2 className="text-2xl font-bold text-white tracking-tight">{selectedExercise.name}</h2>
                <p className="text-xs text-slate-400 mt-0.5">{selectedExercise.equipment} • {selectedExercise.primaryMuscles.join(', ')}</p>
              </div>
            ...
            <div className="space-y-3">
              <h3 className="text-xs font-bold text-slate-300 uppercase tracking-wider flex items-center gap-2">
                <ImageIcon className="w-4 h-4 text-teal-400" /> Generated Media Sets ({selectedExercise.media.length} Views)
              </h3>

              <div className="grid sm:grid-cols-2 gap-4">
                {selectedExercise.media.map((media) => (
                  <div key={media.id} className="bg-slate-950 border border-slate-800 rounded-2xl overflow-hidden p-3 space-y-2">
                    <div className="aspect-[4/3] rounded-xl overflow-hidden bg-slate-900 border border-slate-800 relative">
                      ...
                      <span className="absolute top-2 left-2 bg-slate-900/90 text-teal-300 text-[10px] font-bold px-2 py-0.5 rounded-full uppercase border border-slate-700">
                    ...
                    <div className="space-y-1">
                      <p className="text-xs font-semibold text-slate-200">{media.caption}</p>
                      {media.prompt && (
                    ...
                    {media.reviewStatus !== 'approved' && (
                      <button
                        ...
                        className="w-full py-1.5 bg-slate-800 hover:bg-slate-750 text-teal-400 text-xs font-bold rounded-lg transition-colors border border-slate-700"
                      >
                        Approve Image
                      </button>
                    )}
            ...
            <div className="space-y-4 border-t border-slate-800 pt-4 text-xs">
              <h3 className="font-bold text-slate-300 uppercase tracking-wider">Instructional Quality Audit</h3>
              
              <div className="space-y-2 bg-slate-950 p-4 rounded-2xl border border-slate-800">
                <span className="font-bold text-teal-400 block">Breathing Cue</span>
                <p className="text-slate-300">{selectedExercise.breathingInstructions}</p>
              </div>
